...
/* --- Caller: a change list where one source code (5012 Snillfjord) splits into */
/* three targets in the same period. PROC SORT NODUPKEY routes the duplicates to  */
/* DUPOUT, the macro lists them, and per its own logic the recode format is not   */
/* built when duplicates exist. behold_utds_dubletter=1 keeps the duplicate set.  */
%EndringFraKlass(
  innfil=changes.csv,
  klass_katnr=131,
  lengde_kode=4,
  behold_utds_dubletter=1,
  utds_dubletter=Klass_endringer_dubl
);

proc print data=work.Klass_endringer_dubl n;
  title "Dubletter rutet til DUPOUT av PROC SORT NODUPKEY";
run;
