...
/* --- Caller: exercise %EndringFraKlass against a bundled change list. */
/* Mock data substitutes the live Klass API fetch; all macro logic runs unchanged. */
%EndringFraKlass(
  innfil=changes.csv,
  klass_katnr=131,
  lengde_kode=4,
  format_omk=$omkode,
  lag_format_omk=1,
  behold_endring_data=1,
  behold_format_data=1
);

/* Show the recode format the macro produced from the change list. */
data demo;
...
proc print data=demo; 
  title "Omkoding via formatet $omkode laget av EndringFraKlass";
run;
