...
/* --- Caller: a change list where a code changes twice in the period (gjengangere). */
/* Exercises the PROC SQL self-join + DATA-step UPDATE loop that collapses chained changes. */
%EndringFraKlass(
  innfil=changes.csv,
  klass_katnr=131,
  lengde_kode=4,
...
/* The chained change 1567 -> 1539 -> 5061 should collapse to 1567 -> 5061. */
data demo;
  length kode $4 nytt_kode $4;
  input kode $;
  nytt_kode = put(kode, $omkode.);
  datalines;
1567
1539
1502
...
proc print data=demo;
  title "Omkoding etter sammenslaaing av flere endringer (gjengangere)";
run;
